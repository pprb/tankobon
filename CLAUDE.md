...
Persistence uses Node's built-in `node:sqlite` (`DatabaseSync`) — no native module to compile, and nothing stored in the cloud or in Chromium's `localStorage`/IndexedDB. The DB file (`tankobon.db`) lives in Electron's `userData` directory by default (`database.ts`), or wherever `db-location.ts` says.

- `library-repository.ts` — one row per comic ever opened (path, title, page count, current page, file count, file size, rating, tags). `touch(filePath, title, pageCount, fileCount, fileSize)` is the upsert used on every `comic:open`: it creates the row on first open or refreshes metadata/clamps `currentPage` if the page count changed — it deliberately never touches `rating`/`tags`, which are user-set and must survive reopening a comic. `updateRating()`/`updateTags()` are the dedicated writers for those two fields. `fileCount` is the archive's total entry count (all files, not just image pages — see `ComicArchive.fileCount` below); `fileSize` is the `.cbz`/`.cbr` file's size on disk, computed via `fs.stat` in `main/ipc/comic.ts` (not inside `ComicArchive`, since size-on-disk isn't something the archive abstraction inherently knows). `tags` is a JSON-encoded `string[]` column — no normalized tags table, since a personal comic library doesn't need one. `lastOpenedPath()` returns the most recently opened comic's path; `main/ipc/comic.ts` turns it into the open dialog's `defaultPath` (falling back to the OS default when there's no history yet or that directory no longer exists), so picking a book starts where the last one was picked.
- `db-location.ts` — where the DB file lives. This can't be an `AppSettings` entry like everything else, since the settings are stored *in* the database and the location has to be known before it's open: it's a tiny `db-location.json` pointer file that always stays in `userData`, holding the directory of the database it points at. A missing/malformed pointer silently falls back to `userData` rather than throwing, so a hand-edited (or sync-mangled) file can't stop the app from starting. The functions are pure (they take the `userData` path, never `app.getPath`), which is what makes them testable; `database.ts` is the only place that bridges them to Electron. The `database:*` IPC handlers (`main/ipc/database.ts`) let the settings page read it, pick a new directory and restart. Changing it only rewrites the pointer — the existing file is deliberately *not* moved, because pointing at a directory that already holds a `tankobon.db` (a synced folder, another machine) must keep that database rather than overwrite it — and the already-open database stays open until the next start.
...
- `import-service.ts` — the other direction, behind `data:import`. `parseExport()` validates a user-picked file (rejecting anything that isn't a `version: 1` export) and sanitizes it field by field, dropping entries too broken to use and keeping only known settings keys whose value type matches the default — an export is a file the user can edit, so nothing in it is trusted. `applyImport()` then merges it in: library entries match on **path**, not id (ids aren't stable across machines), and the snapshot wins for the entries it contains — including `rating`/`tags`, unlike `touch`, since restoring a backup is exactly what the user asked for. Entries only present locally are left untouched; settings are replaced wholesale. Validation failures come back as `{ status: 'error', message }` in the `ImportResult` union (`src/shared/data.ts`) rather than as a thrown error, because an `ipcMain.handle` rejection reaches the renderer wrapped in "Error invoking remote method …".
...
### Folder scanning (`src/main/services/library-scanner.ts`)

Besides picking one file, the library page can add a whole tree: `library:add-folder` opens a directory picker, then `scanIntoLibrary()` walks it recursively for `.cbz`/`.cbr`/`.pdf` and registers everything it finds. Directories that can't be read are skipped instead of failing the scan, and symlinks are never followed (`Dirent.isDirectory()` is false for them, which also rules out cycles).

Learning a comic's page count means actually opening it, which is the slow part — hence the progress bar. The scan reports file by file over `library:scan-progress` (main → renderer, the one push channel in the app: everything else is `invoke`/`handle`). The renderer subscribes via `library.onScanProgress()`, which returns its own unsubscribe function, for the whole life of the library page rather than around each scan — the main process starts sending progress as soon as the directory is picked, well before `addFolder()` resolves. A file that won't open only costs itself (`failed` in the summary), never the rest of the scan.

Scanned files go through `LibraryRepository.register()`, **not** `touch()`: a scan is not a read, so an entry that already exists is left completely alone (no `last_opened_at` bump, no metadata refresh, no progress reset), and `hasPath()` lets the scanner skip those without paying to open them. New rows get `last_opened_at = added_at`, which is what puts a freshly scanned batch at the top of the library list. `openArchive()` is factored out of `ComicService` for this: the scanner opens each file just long enough to read `pages.length`/`fileCount` and closes it again, with no use for the service's keep-alive map.

### Settings sections (`src/routes/settings.tsx` + `src/routes/settings/`)

The settings are split across sub-routes rather than one long screen: `settings.tsx` is a layout route (title + `<Outlet/>`), and `settings/reading.tsx`, `settings/appearance.tsx`, `settings/data.tsx` are the sections. `settings/index.tsx` has no component — it `redirect`s `/settings` to the first section, so the sidebar entry still has somewhere to point. The section list lives in `src/lib/settings-nav.ts` (`SETTINGS_SECTIONS`), shared with the sidebar so the two can't drift.

In the sidebar (`__root.tsx`), `SettingsNav` unfolds those sections under "Paramètres": clicking it while folded unfolds *and* navigates, clicking it while unfolded only folds (it `preventDefault()`s the `Link`, since navigating would drag the user off the section they're on). The unfolded state is transient component state, not an `AppSettings` entry, and the collapsed icon-rail hides the sub-entries entirely. Adding a section means a file in `src/routes/settings/` plus a line in `SETTINGS_SECTIONS` — and `src/routeTree.gen.ts` regenerating, which the Vite plugin does on the next `npm start`/build.

### Library search & filters (`src/lib/library-filter.ts`)

The library page's toolbar (`src/routes/index.tsx`) narrows the list client-side — the whole library is a single `library:list` IPC call and a personal collection is small, so there's no SQL-side filtering. All the logic is pure and lives in `src/lib/library-filter.ts` (hence unit-testable under Vitest's `node` environment, which has no DOM):

- search matches the title *and* the file path (so a series folder name finds its albums), normalized with `NFD` + diacritic stripping so "pokemon" matches "Pokémon";
- read/unread is a **tag** filter, not a progress-derived status — `Lu`/`À lire` are just the `QUICK_TAGS` the entries already carry, so filtering uses the same vocabulary the user tags with. `availableTags()` builds the chip row from the tags in use, keeping the quick tags pinned in front (stable position even before anything carries them); selecting several requires *all* of them;
...
`ContinuousReader` renders one `ContinuousPage` per page index; each has two `IntersectionObserver`s: one with a large `rootMargin` that lazily fetches the page (`comic.readPage`) once it's getting close, and one with `threshold: 0.5` that reports the page as "active" for the header's counter and for persisting reading progress (debounced ~400ms, calling `library.updateProgress` directly — deliberately *not* `useComic`'s `goTo`, which would also kick off its own single-page fetch for a `pageUrl` continuous mode never uses). Zoom, AI upscaling, and RTL page-turn direction don't apply here; pages always render top-to-bottom at container width. Opening a comic resumes at `comic.resumePage` like single-page mode does: `useResumeScroll` scrolls to it in a layout effect (before the observers' first async callbacks, so page 0 never gets reported and persisted over the saved progress), then keeps it pinned to the top via a `ResizeObserver` on the page column — every page starts as a fixed-height placeholder, and the preloaded pages above it change height once their image arrives — until the user's first wheel/pointer/touch/key input. `ContinuousReader` is keyed by `comic.id` so opening another book remounts it and resumes that book too.

### Wheel-to-turn-page (single-page mode)

The reader's wheel handler (`src/routes/reader.tsx`, `SinglePageReader`) only turns the page once there's nothing left to scroll in that direction: in "fit" zoom it always turns the page (no scrollable content); when zoomed in, it checks the container's `scrollTop`/`scrollHeight` first and lets the native scroll pan the image, only turning the page once panning is already at that edge. `AppSettings.scrollDirection` (`standard`/`inverted`) picks whether scrolling down calls `advance()` or `retreat()`. `createWheelPager()` (`src/lib/wheel-pager.ts`, pure and unit-tested) collapses one trackpad swipe's many `wheel` events into a single page turn — including macOS's inertia tail, which keeps emitting decaying events for a second or two after the finger lifts (the fixed ~450ms cooldown used before turned 2-3 pages per swipe — worse, it lived inside the wheel effect, which re-runs after every page turn because `useComic`'s `next`/`prev` change identity each render, so it was reset right after each turn; the pager is now held in `useState` so the gesture survives re-renders). A gesture only ends after ~200ms without any wheel event, except that a delta jumping well above the previous one (inertia only decays) counts as a fresh swipe. Panning events are fed to it too, so a swipe that pans to the edge of a zoomed page doesn't turn it with its leftover inertia. Note for testing: a JS-dispatched `WheelEvent` fires listeners but does not actually scroll the element (no native default action) — verifying real scroll behavior needs CDP's `Input.dispatchMouseEvent` with `type: 'mouseWheel'`, not `element.dispatchEvent(new WheelEvent(...))`.
